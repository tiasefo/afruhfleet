...
export function TrackingSearch() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    // Demo: show result if tracking number contains "AFR" or "12847"
    if (searchValue.toUpperCase().includes('AFR') || searchValue.includes('12847')) {
      setShowResult(true)
    } else {
      setNotFound(true)
    }
  ...
  const status = statusConfig[demoShipment.status as keyof typeof statusConfig]
  const paymentStatus = paymentStatusConfig[demoShipment.paymentStatus as keyof typeof paymentStatusConfig]
              ...
              <CardHeader>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <CardTitle className="text-2xl">{demoShipment.trackingNumber}</CardTitle>
                  </div>
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-between rounded-lg bg-muted/50 p-4">
                  <div className="text-center">
                    ...
                    <p className="font-semibold">{demoShipment.origin.city}</p>
                    <p className="text-sm text-muted-foreground">{demoShipment.origin.country}</p>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <p className="font-semibold">{demoShipment.destination.city}</p>
                    <p className="text-sm text-muted-foreground">{demoShipment.destination.country}</p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-4 sm:grid-cols-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{formatDate(demoShipment.origin.date)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{formatDate(demoShipment.destination.date)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium">{demoShipment.packageCount} pkgs ({demoShipment.weight} kg)</p>
                    </div>
                  </div>
                </div>
              </CardContent>
              ...
              <Card className="lg:col-span-2">
                ...
                <CardContent>
                  <div className="relative space-y-0">
                    {demoShipment.events.map((event, index) => (
                      <div key={event.id} className="relative flex gap-4 pb-8 last:pb-0">
                        {/* Timeline Line */}
                        {index < demoShipment.events.length - 1 && (
              ...
              <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <p className="font-medium">{demoShipment.sender.name}</p>
                    <p className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Phone className="h-4 w-4" />
                      {demoShipment.sender.phone}
                    </p>
                    <p className="flex items-center gap-2 text-sm text-muted-foreground">
                      <MapPin className="h-4 w-4" />
                      {demoShipment.sender.city}, {demoShipment.sender.country}
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <p className="font-medium">{demoShipment.receiver.name}</p>
                    <p className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Phone className="h-4 w-4" />
                      {demoShipment.receiver.phone}
                    </p>
                    <p className="flex items-center gap-2 text-sm text-muted-foreground">
                      <MapPin className="h-4 w-4" />
                      {demoShipment.receiver.city}, {demoShipment.receiver.country}
                    </p>
                  </CardContent>
                </Card>
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          {demoShipment.payment.currency} {demoShipment.payment.total.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Paid</span>
                        <span className="text-chart-4">
                          {demoShipment.payment.currency} {demoShipment.payment.paid.toLocaleString()}
                        </span>
                      </div>
                      <div className="border-t pt-3">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold text-destructive">
                            {demoShipment.payment.currency} {demoShipment.payment.balance.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
