...
export function VendorRegistration() {
  ...
  const handleSubmit = async () => {
    ...
    if (!apiBaseUrl) {
      setSubmitError('API base URL is not configured. Set NEXT_PUBLIC_API_BASE_URL.')
      setIsSubmitting(false)
      return
    }

    try {
      const response = await fetch(joinApiUrl(apiBaseUrl, '/vendors/register'), {
        method: 'POST',
        headers: {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Vendor registration failed')
    } finally {
      setIsSubmitting(false)
    }
  }
