...
export default function VendorActionsDashboardPage() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const data = await api.get('/sentinel/vendors?status=pending&page=1&page_size=50')
  ...
  const approve = async (id: string) => {
    try {
      await api.post(`/sentinel/vendors/${id}/review`, { action: 'approve' })
  ...
  const reject = async (id: string) => {
    ...
    try {
      await api.post(`/sentinel/vendors/${id}/review`, { action: 'reject', rejection_reason: reason })
