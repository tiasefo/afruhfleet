...
export default function TemplatesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await api.get<Template[]>('/sentinel/templates/admin/all')
      setTemplates(data)
    } catch (err) {
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      await api.delete(`/sentinel/templates/admin/${id}`)
  ...
  const handleToggle = async (t: Template) => {
    try {
      const updated = await api.patch<Template>(`/sentinel/templates/admin/${t.id}`, {
...
function TemplateEditor({
  ...
  const handleSave = async () => {
    ...
    try {
      if (template) {
        await api.patch(`/sentinel/templates/admin/${template.id}`, {
          name, description, preset,
        })
      } else {
        await api.post('/sentinel/templates/admin', {
