from __future__ import annotations

from app.plugins import BasePlugin
from app.services.tenant_branding_service import ensure_tenant_branding

from sqlalchemy.orm import Session


class TrackingPlugin(BasePlugin):
    name = "tracking"
    feature_name = "Shipment Tracking"
    required_endpoints = [
        "GET /api/v1/shipments/{tenant_id}/track",
        "GET /api/v1/shipments/public/track/{tenant_id}/{tracking_number}",
    ]
    feature_flags = ["public_tracking_enabled"]

    def check(self, tenant_id: str, db: Session) -> bool:
        branding = ensure_tenant_branding(db, tenant_id, "", "")
        return getattr(branding, "public_tracking_enabled", False)

    def auto_fix(self, tenant_id: str, db: Session) -> tuple[bool, str]:
        branding = ensure_tenant_branding(db, tenant_id, "", "")
        if not getattr(branding, "public_tracking_enabled", False):
            branding.public_tracking_enabled = True
            db.add(branding)
            db.commit()
            return True, "Enabled public_tracking_enabled on TenantBranding"
        return True, "Already enabled"

    def get_health(self, tenant_id: str, db: Session) -> dict:
        healthy = self.check(tenant_id, db)
        return {
            "plugin": self.name,
            "healthy": healthy,
            "endpoints": self.required_endpoints,
            "details": "Tracking endpoints are route-mounted and always available" if healthy else "public_tracking_enabled is False",
            "auto_fixed": False,
        }


PLUGIN = TrackingPlugin()
