...
def calculate_route(tenant_id: str, origin_lat: float=Query(...), origin_lng: float=Query(...), dest_lat: float=Query(...), dest_lng: float=Query(...), origin_city: str | None=Query(None), dest_city: str | None=Query(None), country: str | None=Query(None), use_priority_service: bool=Query(False), current_user: User=Depends(require_active_subscription)):
    ...
    if use_priority_service:
        ...
        result = geo_service.calculate_route((origin_lat, origin_lng), (dest_lat, dest_lng))
    if not result:
...
def get_active_shipment_routes(tenant_id: str, use_priority_service: bool=Query(False), db: Session=Depends(get_db), current_user: User=Depends(require_active_subscription)):
    ...
    for s in shipments:
        if use_priority_service:
            origin = global_geo_service.geocode_country_city_region(s.origin_city, s.origin_country)
            dest = global_geo_service.geocode_country_city_region(s.destination_city, s.destination_country)
        else:
        ...
        if origin and dest:
            ...
            routes.append({'shipment_id': str(s.id), 'tracking_number': s.tracking_number, 'status': s.status.value, 'sender_name': s.sender_name, 'receiver_name': s.receiver_name, 'estimated_arrival': s.estimated_arrival.isoformat() if s.estimated_arrival else None, 'origin': {'lat': origin['lat'], 'lng': origin['lng'], 'city': s.origin_city, 'country': s.origin_country}, 'destination': {'lat': dest['lat'], 'lng': dest['lng'], 'city': s.destination_city, 'country': s.destination_country}})
    logger.info('Generated %d shipment routes for tenant %s (geocoded: %d/%d)', len(routes), tenant_id, successful_geocodes, len(shipments))
