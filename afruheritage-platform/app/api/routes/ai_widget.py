...
def get_widget_config(request: Request, host: str | None=Query(None), db: Session=Depends(get_db)) -> AIWidgetConfigResponse:
    ...
    if effective_host in MOTHER_HOSTS:
        return AIWidgetConfigResponse(enabled=True, tenant_slug=None, model='afruheritage-copilot:latest', scope='shared', welcome_message='Welcome to Afruheritage Assistant. How can I help you today?', theme='light', primary_color='#0ea5e9', api_endpoint='/api/v1/ai/chat')
    ...
    if settings:
        ...
        return AIWidgetConfigResponse(enabled=settings.widget_enabled, tenant_slug=tenant_slug, model=settings.chat_model, scope=settings.retrieval_scope, welcome_message=settings.welcome_message, theme=settings.theme, primary_color=settings.primary_color, api_endpoint='/api/v1/ai/chat')

    tenant_slug = effective_host.split('.')[0] if '.' in effective_host else effective_host
    if not tenant_slug:
        tenant_slug = 'afruheritage'
    return AIWidgetConfigResponse(enabled=True, tenant_slug=tenant_slug, model='afruheritage-copilot:latest', scope=f'tenant:{tenant_slug}', welcome_message=f'Welcome to {tenant_slug} Assistant. How can I help you today?', theme='light', primary_color='#0ea5e9', api_endpoint='/api/v1/ai/chat')
