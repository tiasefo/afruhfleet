...
export default function FleetbaseSummaryPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/api/v1/fleetbase-proxy/fleets', { cache: 'no-store' })
        ...
        setRows(Array.isArray(data?.fleets) ? data.fleets : [])
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="bg-[#063f4f] px-6 py-20 text-white">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="mt-3 text-4xl font-bold">Fleetbase Fleets</h1>
          <p className="mt-4 max-w-2xl text-white/80">Live fleet records from Fleetbase FleetOps.</p>
                ...
                <table className="w-full text-left text-sm">
                  <thead className="bg-slate-50 text-slate-600">
                    <tr>
                      ...
                      <th className="px-4 py-3 font-semibold">Drivers Online</th>
                      <th className="px-4 py-3 font-semibold">Vehicles Online</th>
                    </tr>
                  </thead>
                  <tbody>
                    {rows.map((row, idx) => (
                      <tr key={row?.uuid || row?.id || idx} className="border-t">
                        <td className="px-4 py-3">{formatValue(row?.name)}</td>
                        <td className="px-4 py-3">{formatValue(row?.status)}</td>
                        <td className="px-4 py-3">{formatValue(row?.drivers_count)}</td>
                        <td className="px-4 py-3">{formatValue(row?.vehicles_count)}</td>
                        <td className="px-4 py-3">{formatValue(row?.drivers_online_count)}</td>
                        <td className="px-4 py-3">{formatValue(row?.vehicles_online_count)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
