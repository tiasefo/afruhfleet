...
export function VendorRegistration() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/vendors/register`, {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          background_check_accepted: formData.backgroundCheckAccepted,
        }),
      })
      if (!res.ok) {
              ...
              <>
                ...
                <div className="rounded-lg border border-dashed border-muted-foreground/30 p-6 text-center">
                  ...
                  <label htmlFor="vehicle-docs" className="cursor-pointer">
                    <span className="mt-3 inline-flex items-center rounded-md border border-input bg-background px-3 py-1.5 text-sm font-medium shadow-sm hover:bg-accent hover:text-accent-foreground">
                      Choose Files
                    </span>
                    <input
                      id="vehicle-docs"
                      ...
                      onChange={(e) => {
                        ...
                        if (files) {
                          ...
                          updateFormData('documentNames', names)
                        }
                      }}
                    />
                  </label>
                  {formData.documentNames && (formData.documentNames as string[]).length > 0 && (
                    <div className="mt-3 space-y-1">
                      {(formData.documentNames as string[]).map((name: string, i: number) => (
                        <p key={i} className="text-xs text-muted-foreground">📎 {name}</p>
                      ))}
                    </div>
                  )}
                  <p className="mt-2 text-xs text-muted-foreground">
                    Documents will be reviewed during the approval process.
                  </p>
                </div>
              </>
            )}
