import { VideoBackdrop } from "@/templates/amooksco/components/video-backdrop"
import { workflow } from "@/lib/amooksco"

export function Workflow() {
  return (
    <section className="relative overflow-hidden border-b border-border bg-card">
      {/* Ambient cargo-port footage blended softly beneath the section */}
      <VideoBackdrop
        src="/tenant-assets/amooksco/hero-shipping.mp4"
        poster="/tenant-assets/amooksco/container-delivery.jpeg"
        videoClassName="opacity-25"
        overlayClassName="bg-gradient-to-b from-background/85 via-background/80 to-background"
      />
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {workflow.map((w, i) => (
            <div key={w.step} className="relative">
              <div className="flex h-full flex-col rounded-xl border border-border bg-card/90 p-6 backdrop-blur-sm">
