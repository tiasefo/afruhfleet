          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto bg-muted/40 px-4 py-4">
            {messages.length === 0 && (
              <div className="space-y-3">
                <div className="rounded-xl rounded-tl-sm bg-card px-3 py-2 text-sm text-foreground shadow-sm">
                  Hi! I&apos;m Amo, your AMOOKSCO assistant. Ask me about shipping from
                  China to Ghana, tracking, or Mobile Money payments.
                </div>
            ...
            {busy && messages[messages.length - 1]?.role === "user" && (
              <div className="flex justify-start">
                <div className="rounded-xl rounded-tl-sm bg-card px-3 py-2 text-sm text-muted-foreground shadow-sm">
                  Typing…
                </div>
              </div>
            )}
            ...
            {error && (
              <div className="flex justify-start">
                <div className="max-w-[85%] space-y-2 rounded-xl rounded-tl-sm border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-foreground shadow-sm">
                  <p>
                    Sorry, I couldn&apos;t reach the assistant right now. Please
                    contact our team directly:
                  </p>
                  <a
                    href={waLink(whatsapp.tracking, "Hello AMOOKSCO, I need assistance.")}
                    target="_blank"
