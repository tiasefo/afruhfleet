...
export default function RuntimesPage() {
  ...
  const lookup = async () => {
    ...
    setSearched(true)
    try {
      const data = await api.get(`/admin/runtime/tenant/${tenantId}`)
      setRuntime(data)
    } catch (e: any) {
  ...
  const handleDeploy = async () => {
    setDeploying(true)
    try {
      await api.post('/admin/runtime/deploy', {
  ...
  const handleSuspend = async () => {
    ...
    try {
      await api.post('/admin/runtime/suspend', { runtime_id: runtime.id })
  ...
  const handleRetry = async () => {
    if (!runtime?.id) return
    try {
      await api.post('/admin/runtime/retry', { runtime_id: runtime.id })
